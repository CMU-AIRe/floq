...
class FloQDistributionalAgent(flax.struct.PyTreeNode):
    ...
    def critic_loss(self, batch,  grad_params, rng,):
        ...
        target_returns = rewards + self.config['discount'] * masks * next_returns #(b,r)

        assert target_returns.shape == (batch_size, repeat)
        ...
        x_1 = jnp.tile(target_returns.reshape((1, batch_size, repeat)), [self.config['flow_num_ensembles'], 1, 1]) #(e, b, r)

        assert x_1.shape == (self.config['flow_num_ensembles'], batch_size, repeat)
        ...
        mean_target_returns = jnp.mean(target_returns, axis = -1) #(batch_size,)
        td_error = (mean_current_returns - mean_target_returns)**2
...
def get_config():
    config = ml_collections.ConfigDict(
        dict(
            agent_name='floqdistributional',  # Agent name.
