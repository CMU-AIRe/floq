...
from utils.networks_plasticity import Value, CriticVectorFieldFeatures, ValueHead, ActorVectorField




class FloQPlasticityAgent(flax.struct.PyTreeNode):
    ...
    def critic_loss_before_freeze(self, batch,  grad_params, rng,):
        ...
        next_returns = self.compute_target_flow_returns_before_freeze(next_observations, 
                                                                      next_actions, 
        ...
        features = self.network.select('floq_features')(observations, actions, returns = x_t, times = t, params = grad_params) #(e, b*r, 1)
        pred = self.network.select('value_head')(features, params = grad_params)
    ...
    def critic_loss_after_freeze(self, batch,  grad_params, rng,):
        ...
        next_returns = self.compute_target_flow_returns_after_freeze(next_observations, 
                                                                     next_actions, 
                                                                     noise_ratios = next_noise_ratios,).squeeze(-1).reshape((batch_size, repeat)) #(b, r)
        ...
        features = self.network.select('floq_features')(observations, actions, returns = x_t, times = t,) #(e, b*r, 1)
    ...
    def update_before_freeze(self, batch,):
        ...
        self.target_update(new_network, 'floq_features')
        self.target_update(new_network, 'value_head')
    ...
    @jax.jit
    def compute_target_flow_returns_before_freeze(
        self,
        observations,
        actions,
        noise_ratios,
        ...
        for i in range(self.config['critic_flow_steps']):    
            ...
            features = self.network.select('target_floq_features')(observations, actions, returns = returns, times = t,) #(flow_num_ensembles, batch_size, 1)
            vels = self.network.select('target_value_head')(features)

            vels = jnp.expand_dims(vels, axis = -1)
    ...
    @jax.jit
    def compute_target_flow_returns_after_freeze(
        self,
        observations,
        actions,
        noise_ratios,
        ...
        for i in range(self.config['critic_flow_steps']):    
            ...
            features = self.network.select('floq_features')(observations, actions, returns = returns, times = t,) #(flow_num_ensembles, batch_size, 1)
            vels = self.network.select('target_value_head')(features)
            vels = jnp.expand_dims(vels, axis = -1)
    ...
    def compute_current_flow_returns(
        ...
        for i in range(self.config['critic_flow_steps']):    
            ...
            features = self.network.select('floq_features')(observations, actions, returns = returns, times = t,) #(flow_num_ensembles, batch_size, 1)
            vels = self.network.select('value_head')(features)
            vels = jnp.expand_dims(vels, axis = -1)
    ...
    def create(
        ...
        if config['encoder'] is not None:
            ...
            encoders['floq_features'] = encoder_module()
            encoders['value_head'] = encoder_module()
        ...
        feature_dim = hidden_dims[-2]
        value_head_hidden_dim = hidden_dims[-1]

        floq_features_def = CriticVectorFieldFeatures(
            hidden_dims=hidden_dims[:-1],
            layer_norm=config['layer_norm'],
            encoder = encoders.get('floq_features'),
        ...
        value_head_def = ValueHead(
            hidden_dims=(value_head_hidden_dim,),
            layer_norm=config['layer_norm'],
            num_ensembles=config['flow_num_ensembles'],
            encoder=encoders.get('value_head'),
        )

        ex_features = jnp.ones((config['flow_num_ensembles'], feature_dim))


        network_info = dict(
            critic=(critic_def, (ex_observations, ex_actions,)),

            floq_features = (floq_features_def, (ex_observations, ex_actions, ex_returns,  critic_ex_times,)),            
            target_floq_features = (copy.deepcopy(floq_features_def), (ex_observations, ex_actions, ex_returns, critic_ex_times,)),

            value_head = (value_head_def, (ex_features,)),
            target_value_head = (copy.deepcopy(value_head_def), (ex_features,)),
        ...
        params['modules_target_floq_features'] = params['modules_floq_features']
        params['modules_target_value_head'] = params['modules_value_head']
...
def get_config():
    config = ml_collections.ConfigDict(
        dict(
            agent_name='floqplasticity',  # Agent name.
