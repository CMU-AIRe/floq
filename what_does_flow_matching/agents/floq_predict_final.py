...
from utils.networks import ActorVectorField, Value, CriticVectorField

import jax.numpy as jnp

class FloQPredictFinalAgent(flax.struct.PyTreeNode):
    ...
    def compute_target_flow_returns(
        ...
        for i in range(self.config['critic_flow_steps']):    
            ...
            final_preds = self.network.select('target_floq')(observations, actions, returns = returns, times = t,) #(flow_num_ensembles, batch_size, 1)
            assert final_preds.ndim  == 3 and final_preds.shape[0] == self.config['flow_num_ensembles']

            #returns = returns + vels/ self.config['critic_flow_steps'] #(flow_num_ensembles, batch_size, 1)
            t_next = (i+ 1.)/self.config['critic_flow_steps']
            returns = (1. - t_next)*initial_returns + t_next*final_preds 
    ...
    def compute_current_flow_returns(
        ...
        for i in range(self.config['critic_flow_steps']):    
            ...
            final_preds = self.network.select('floq')(observations, actions, returns = returns, times = t,) #(flow_num_ensembles, batch_size, 1)
            assert final_preds.ndim  == 3 and final_preds.shape[0] == self.config['flow_num_ensembles']
...
def get_config():
    config = ml_collections.ConfigDict(
        dict(
            agent_name='floqpredictfinal',  # Agent name.
