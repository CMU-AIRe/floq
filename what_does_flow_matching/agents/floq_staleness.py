...
class FloQStalenessAgent(flax.struct.PyTreeNode):
    ...
    def critic_loss(self, batch,  grad_params, rng,):
        ...
        old_returns = self.compute_old_flow_returns(observations, 
                                                    actions, 
                                                    noise_ratios = noise_ratios,) #(e, b*r, 1) where e=num_ensembles

        old_returns = old_returns.reshape((self.config['flow_num_ensembles'], batch_size, repeat)) #(e, b, r)


        new_returns = self.compute_new_flow_returns(observations, 
                                                    actions, 
                                                    noise_ratios = noise_ratios,) #(e, b*r, 1) where e=num_ensembles

        new_returns = new_returns.reshape((self.config['flow_num_ensembles'], batch_size, repeat)) #(e, b, r)


        dist_old_current = jnp.sqrt((jnp.mean(old_returns, axis = -1) - jnp.mean(current_returns, axis = -1))**2/(jnp.var(old_returns, axis = -1) + 1e-8)) #(e,b)
        dist_new_current = jnp.sqrt((jnp.mean(new_returns, axis = -1) - jnp.mean(current_returns, axis = -1))**2/(jnp.var(new_returns, axis = -1) + 1e-8)) #(e,b)
        ...
        return critic_loss.mean(), {
            ...
            'dist_old_current': dist_old_current.mean(),
            'dist_new_current': dist_new_current.mean(),
        }
    ...
    def old_update(self, network, module_name):
        """Update the target network."""
        
        new_target_params = jax.tree_util.tree_map(
            lambda p, tp: p * 1. + tp * 0.,
            self.network.params[f'modules_{module_name}'],
            self.network.params[f'modules_old_{module_name}'],
        )
        
        network.params[f'modules_old_{module_name}'] = new_target_params

    @jax.jit
    def update_before(self, batch,):
        ...
        self.old_update(new_network, 'floq')
        self.old_update(new_network, 'target_floq')

        return self.replace(network=new_network, rng=new_rng), info

    @jax.jit
    def update_after(self, batch,):
        """Update the agent and return a new agent with information dictionary."""
        new_rng, rng = jax.random.split(self.rng)
    ...
    def compute_target_flow_returns(
        ...
        for i in range(self.config['critic_flow_steps']):    
            ...
            if i < self.config['old_network_integration_steps']:
                vels = self.network.select('old_target_floq')(observations, actions, returns = returns, times = t,) #(flow_num_ensembles, batch_size, 1)
            else:
    ...
    def compute_current_flow_returns(
        ...
        for i in range(self.config['critic_flow_steps']):    
            ...
            if i < self.config['old_network_integration_steps']:
                vels = self.network.select('old_floq')(observations, actions, returns = returns, times = t,) #(flow_num_ensembles, batch_size, 1)
    ...
    def create(
        ...
        network_info = dict(
            ...
            old_floq = (copy.deepcopy(floq_def), (ex_observations, ex_actions, ex_returns, critic_ex_times,)),
            old_target_floq = (copy.deepcopy(floq_def), (ex_observations, ex_actions, ex_returns, critic_ex_times,)),
        ...
        params['modules_old_floq'] = params['modules_floq']
        params['modules_old_target_floq'] = params['modules_floq']
...
def get_config():
    config = ml_collections.ConfigDict(
        dict(
            agent_name='floqstaleness',  # Agent name.
            ...
            old_network_integration_steps=4, #Number of integration steps with the old network.
        )
    )
    return config