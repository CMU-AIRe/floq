...
class FloQTargetNoiseAgent(flax.struct.PyTreeNode):
    ...
    def critic_loss(self, batch,  grad_params, rng,):
        ...
        target_noise = self.config['target_noise_magnitude']*(2.*jax.random.uniform(target_noise_rng, vel.shape) - 1.)

        vel = vel + target_noise
...
def get_config():
    config = ml_collections.ConfigDict(
        dict(
            agent_name='floqtargetnoise',  # Agent name.
