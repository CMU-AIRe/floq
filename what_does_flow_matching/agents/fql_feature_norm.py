...
from utils.networks_feature_norm import ActorVectorField, Value, ResValue


class FQLFeatureNormAgent(flax.struct.PyTreeNode):
    ...
    def critic_loss(self, batch, grad_params, rng):
        ...
        if self.config['training_type'] == 'mc':
            target_q = batch['mc_returns'] #(batch_size,)
        else:
            target_q = batch['rewards'] + self.config['discount'] * batch['masks'] * next_q
        
        assert target_q.shape == (batch['rewards'].shape[0],)
    ...
    def actor_loss(self, batch, grad_params, rng):
        ...
        if self.config['training_type'] == 'ql':
            actor_loss = bc_flow_loss.mean() + self.config['alpha'] * distill_loss + q_loss
        else:
...
def get_config():
    config = ml_collections.ConfigDict(
        dict(
            agent_name='fqlfeaturenorm',  # Agent name.
