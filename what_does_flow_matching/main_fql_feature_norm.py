...
def main(_):
    ...
    setup_wandb(project='final_release_checks_fql_feature_norm', group=FLAGS.run_group, name=exp_name, mode = "online")
    ...
    for i in tqdm.tqdm(range(1, FLAGS.offline_steps + FLAGS.online_steps + 1), smoothing=0.1, dynamic_ncols=True):
        ...
        if i % FLAGS.log_interval == 0:
            ...
            if val_dataset is not None:
                ...
                v, feature_norm_stats = agent.network.select('critic')(obs, actions, return_stats=True)

                train_metrics.update({f'feature_norm/{k}': v for k, v in feature_norm_stats.items()})                
