...
flags.DEFINE_integer('online_steps', 120000, 'Number of online steps.')
...
flags.DEFINE_integer('utd_ratio', 1, 'Whether to use balanced sampling for online fine-tuning.')
...
def main(_):
    ...
    setup_wandb(project='final_release_checks_rlpd', group=FLAGS.run_group, name=exp_name, mode = "online")
    ...
    for i in tqdm.tqdm(range(1, FLAGS.online_steps + 1), smoothing=0.1, dynamic_ncols=True):        
        ...
        for _ in range(FLAGS.utd_ratio):
            ...
            if FLAGS.balanced_sampling and (i >= config['batch_size']//2 + 2):
                raise
