...
class MLP(nn.Module):
    ...
    def __call__(self, x):
        for i, size in enumerate(self.hidden_dims):
            x = nn.Dense(size, kernel_init=self.kernel_init, name=f"dense_{i}")(x)
...
class Value(nn.Module):
    ...
    def __call__(self, observations, actions=None, return_stats: bool = False):
        ...
        v, aux = self.value_net.apply(
            {"params": self.value_net.variables["params"]},
            inputs,
            mutable=["intermediates"],
        )

        intermediates = aux["intermediates"]
        stats = {}

        for layer_name, feats in intermediates.items():
            # feats is a tuple; concat over calls
            feats = jnp.concatenate(feats, axis=0)

            # ensemble-aware norm
            stats[f"feature_norm/{layer_name}"] = jnp.mean(
                jnp.linalg.norm(feats, axis=-1)
            )
...
class CriticVectorField(nn.Module):
    ...
    @nn.compact
    def __call__(self, observations, actions, returns=None, times=None, is_encoded=False, return_stats:bool=False):
        ...
        v, aux = self.mlp.apply(
            {"params": self.mlp.variables["params"]},
            inputs,
            mutable=["intermediates"],
        )
...
def collect_feature_norms(intermediates, prefix=""):
    stats = {}
    ...
    for key, value in intermediates.items():
        ...
        if isinstance(value, dict):
            ...
            stats.update(collect_feature_norms(value, name))
        else:
            # value is a tuple of arrays
            feats = jnp.concatenate(value, axis=0)
            stats[f"feature_norm/{name}"] = jnp.mean(
                jnp.linalg.norm(feats, axis=-1)
            )
...
class CriticResVectorField(nn.Module):
    ...
    @nn.compact
    def __call__(self, observations, actions, returns, times=None, is_encoded = False, return_stats:bool=False):
        """Return the vectors at the given states, actions, returns, and times (optional).
        ...
        if not return_stats:
            v = self.mlp(inputs) #(num_ensembles, batch_size)
            v = jnp.expand_dims(v, axis = -1) #(num_ensembles, batch_size, 1)
            return v
        ...
        intermediates = aux["intermediates"]
        stats = collect_feature_norms(intermediates)
