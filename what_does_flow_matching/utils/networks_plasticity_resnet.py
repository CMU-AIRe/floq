...
class ResMLPFeatures(nn.Module):
    ...
    def setup(self):
        ...
        hidden_blocks = [mlp_class(hidden_dims = (*self.per_transformation_hidden_dims,), 
                                   activate_final=True, 
                                   layer_norm=self.layer_norm, 
                                   activate_before_layernorm = self.activate_before_layernorm) for _ in range(self.num_transformations - 1)]

        self.initial_block = initial_block
        self.hidden_blocks = hidden_blocks

        self.final_block = mlp_class(hidden_dims = (*self.per_transformation_hidden_dims[:-1],), 
                                     activate_final=True, 
                                     layer_norm=self.layer_norm, 
                                     activate_before_layernorm = self.activate_before_layernorm)
    ...
    def __call__(self, inputs):
        ...
        for i in range(self.num_transformations - 1):
            
            inputs = self.hidden_blocks[i](inputs) + inputs #(batch_size, latent_dim)
        
        penultimate = inputs
        
        inputs = self.final_block(inputs)

        return inputs, penultimate




class ResValueFeatures(nn.Module):
    ...
    def __call__(self, observations, actions=None):
        ...
        v, penultimate = self.value_net(inputs)
        
        return v, penultimate
...
class ValueHead(nn.Module):
    ...
    def setup(self):
        ...
        self.inner_value_net = mlp_class(
            (*self.hidden_dims,),
            activate_final=True,
            layer_norm=self.layer_norm,
        )

        self.linear_layer = mlp_class(
            (self.output_dim,),
            activate_final=False,
            layer_norm=False,
        )

    def __call__(self, features, penultimate):

        assert features.shape[0] == self.num_ensembles

        inputs = features
        
        inputs = self.inner_value_net(inputs) + penultimate
        v = self.linear_layer(inputs)
